...
with col1:
    if os.path.exists(logo_path):
        ...
        st.title("📌") # Ikon sementara jika logo tiada
with col2:
...
def check_password():
    ...
    if not st.session_state["password_correct"]:
        ...
        with cols[1]:
            st.info("Sila log masuk untuk memulakan.")
    ...
    if uploaded_file is not None:
        try:
            ...
            if not {'STN', 'E', 'N'}.issubset(df.columns):
                st.error("Ralat: Fail CSV mesti ada kolum STN, E, N!")
            else:
                ...
                if show_grid:
                    ...
                    for x in np.arange(np.floor(min_e/grid_spacing)*grid_spacing, max_e, grid_spacing):
                        ...
                        fig.add_trace(go.Scattermapbox(lat=glats, lon=glons, mode='lines', line=dict(width=1, color='rgba(255,255,255,0.3)'), hoverinfo='none', showlegend=False))
                ...
                # 3. Tambah Label (Bearing, Jarak, STN)
                if show_brg_dist:
                ...
                if show_area:
                    fig.add_trace(go.Scattermapbox(lat=[center_lat], lon=[center_lon], mode='text', text=[f"<b>LUAS:<br>{area:.2f} m²</b>"], textfont=dict(size=18, color="yellow")))

                # --- LAYOUT MAPBOX (FIX PETA HILANG) ---
                layers = []
                if show_satellite:
                ...
                fig.update_layout(
                    mapbox=dict(
                        style="white-bg", # Sentiasa white-bg supaya tak perlu token
                        layers=layers,
                ...
                # --- JADUAL DATA (DI BAWAH PETA) ---
                st.markdown("### 📋 Jadual Koordinat")
        ...
        st.info("Sila muat naik fail CSV untuk memaparkan peta.")
