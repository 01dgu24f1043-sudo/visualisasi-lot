...
with st.sidebar:
    ...
    f_opac = st.slider("Kepekatan (Opacity)", 0.0, 1.0, 0.3)
...
if uploaded_file:
    try:
        ...
        points = []
        features_for_geojson = [] # Untuk QGIS
        total_dist = 0
        
        # 1. Proses Data untuk Map & GeoJSON (Titik & Garisan)
        for i in range(len(df)):
            ...
            # --- GEOJSON: Tambah Feature Titik Stesen ---
            features_for_geojson.append({
                ...
                "properties": {
                    ...
                    "N": p1['N'],
                    "E": p1['E'],
                    "Label": f"STN {int(p1['STN'])}"
                }
            })

            # --- GEOJSON: Tambah Feature Garisan (Sempadan) ---
            features_for_geojson.append({
                "type": "Feature",
                ...
                "properties": {
                    "Dari": int(p1['STN']),
                    "Ke": int(p2['STN']),
                    "Bearing_DMS": decimal_to_dms(brg),
                    "Jarak_m": round(dist, 3)
                }
            })

            # Marker Stesen di Folium
            ...
            folium.CircleMarker(
                ...
                tooltip=f"Stesen {int(p1['STN'])}", z_index_offset=1000 
            ).add_to(m)
            ...
            # Bearing & Jarak di Folium
            if show_brg:
                calc_angle = brg - 90
                if 90 < brg < 270: calc_angle -= 180 
                l_html = f'''<div style="transform: rotate({calc_angle}deg); display: flex; flex-direction: column; justify-content: center; align-items: center; color: #00FFFF; font-weight: bold; font-size: {size_brg}pt; text-shadow: 2px 2px 4px black; width: 200px; margin-left: -100px; height: {text_gap}px; margin-top: -{text_gap/2}px; pointer-events: none; text-align: center;">
        ...
        features_for_geojson.append({
            ...
            "properties": {
                ...
                "Info": "Kawasan Lot PUO"
            }
        })

        if show_poly:
            poly_info = f"<div style='width:150px;'><b>INFO LOT</b><hr><b>Luas:</b> {area:.3f} m²<br><b>Perimeter:</b> {total_dist:.3f} m</div>"
            folium.Polygon(
